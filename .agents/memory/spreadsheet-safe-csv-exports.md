...
All data extracted from AutoCAD must be treated as engineering text, never as a spreadsheet formula or spreadsheet error. Values that begin with `=`, `+`, `-`, `@`, or `#` must be exported as spreadsheet-safe text, then restored before DXF write-back.
